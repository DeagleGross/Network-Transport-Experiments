...
An accepted server connection may first run `ObserveTlsClientHelloAsync` under a caller-supplied pre-handshake timeout. The provider reads/parses and caches the first record, invokes the callback, and retains the bytes/state for later authentication. This is the mapping for Kestrel's `UseTlsClientHelloListener`.

For a server connection:

1. The TCP connection is accepted and assigned stable connection identity.
2. The provider reuses a pre-observed ClientHello or reads enough ciphertext to identify the first TLS record and parse `SslClientHelloInfo`.
...
| Obsolete `TlsClientHelloBytesCallback` | Inside the HTTPS handshake timeout; synchronous; first TLS record bytes | `ReadOnlySequence<byte>` borrowed; callback must copy retained bytes; exception fails connection | `TransportServerAuthenticationOptions.ClientHelloCallback` | Exact through pre-read listener | Adapt; runtime fd session must expose captured bytes or provider must safely peek | Exact through ciphertext input before BIO feed | Adapt; fd-bound capture is required | Adapt by observing ciphertext before `AcceptSecurityContext` |
| `UseTlsClientHelloListener` | Connection middleware before `UseHttps`; currently uses the same first-record parser | Has a separate timeout additive to the HTTPS handshake timeout; borrowed bytes; callback is synchronous today | `TransportConnection.ObserveTlsClientHelloAsync` with Kestrel's listener timeout, followed by authentication with a fresh handshake timeout | Exact | Adapt by parking a partially initialized fd-bound TLS handshake or retaining pre-read bytes | Exact through retained ciphertext input | Adapt through poll-driven ClientHello suspension | Adapt by retaining and replaying the first record to `AcceptSecurityContext` |
...
| `ClientCertificateMode.DelayCertificate` | Initial handshake omits client cert; application may request later | Kestrel forbids it for HTTP/2 and calls `SslStream.NegotiateClientCertificateAsync` on supported platforms | `AllowPostHandshakeClientAuthentication` plus `RequestClientCertificateAsync` | Exact subject to current OS support | Fallback/Reject until native PHA or renegotiation is implemented | Possible but unimplemented; provider must handle protocol/version semantics | Fallback/Reject, especially with kTLS | Adapt through runtime Schannel; no claim until tested |
...
If `ObserveTlsClientHelloAsync` ran first, the normalizer and TLS engine consume the cached ClientHello state. The transport must not read a second ClientHello from the network or omit the bytes from authentication.
...
| `ITlsConnectionFeature.GetClientCertificateAsync` | `RequestClientCertificateAsync` when delayed mode is enabled | Unsupported provider throws; Kestrel may choose fallback |
