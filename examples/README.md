...
| `ServerPushApplication.cs` | Persistent `TransportConnection` handles retained in a registry and used for serialized writes outside callbacks |
...
`ServerPushApplication.cs` captures each heap-allocated `TransportConnection` in `OnReady`, wraps it to serialize writes, and uses it later from `Broadcast` without waiting for another transport callback. `OnClosed` removes and marks the wrapper terminal.
